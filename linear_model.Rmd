...
test1 %>% broom::tidy() %>% knitr::kable(digits = 3)
```
...
**Hypothesize: there is significant difference between our model prediction of PCEs for pre- and in-pandemic periods (year 2019, year 2020)**
...
boot_straps_19$strap[[1]] %>% knitr::kable(caption = "A bootstrap example for year 2019")
boot_straps_20$strap[[1]] %>% knitr::kable(caption = "A bootstrap example for year 2020")
```
...
rmse_20 <- boot_straps_20 %>%
  ...
  select(rmse)  
# Perform test
t.test(rmse_19,rmse_20) %>% broom::tidy() %>% knitr::kable(caption = "T-Test Table: Welch Two Sample t-test")
```
